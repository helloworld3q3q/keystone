		...
		return (
			<Container style={{ paddingTop: '2em' }}>
				<ListHeaderTitle
					...
					count = {numeral(items.count).format()}
					title={plural}
				/>
				<ListHeaderToolbar
					// common
	...
	renderBlankState () {
		...
		const button = !currentList.nocreate ? (
			<GlyphButton color="success" glyph="plus" position="left" onClick={onClick} data-e2e-list-create-button="no-results">
				<FormattedMessage id={'create'}/><FormattedMessage id={currentList.singular}/>
			</GlyphButton>
		) : null;
		...
		return (
			<Container>
				...
				<BlankState heading={<FormattedMessage id={"Nones"}/>} style={{ marginTop: 40 }}>
					{button}
				</BlankState>
			</Container>
		);
